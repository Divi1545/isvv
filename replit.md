...
- January 23, 2026. Security hardening for public API - Fixed Stripe webhook to enforce signature verification (rejects when STRIPE_WEBHOOK_SECRET is missing); Fixed server-side pricing to calculate total from service.basePrice * guestCount (prevents client price manipulation); Updated booking/checkout endpoints to use correct Supabase snake_case field names (payment_status, total_price, start_date, services.name).
